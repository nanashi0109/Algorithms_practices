...
@pytest.mark.parametrize("count_removes, expected_result, count_person",
                         [
                             (1, "Name_3 -> Name_2 -> None", 2),
                             (2, "Name_3 -> None", 1),
                             (3, "None", 0),
                         ])
def test_remove_last_person_positive_1(count_removes, expected_result, count_person, pre_condition):
    for i in range(0, count_removes, 1):
        pre_condition.remove_last_person()
...
def test_remove_last_person_positive_2(count_removes, expected_result, pre_condition):
    for i in range(0, count_removes, 1):
        pre_condition.remove_last_person()

    assert pre_condition.remove_last_person().name == expected_result
...
def test_remove_last_person_negative(count_removes, expected_result, count_person, pre_condition):
    for i in range(0, count_removes, 1):
        pre_condition.remove_last_person()
